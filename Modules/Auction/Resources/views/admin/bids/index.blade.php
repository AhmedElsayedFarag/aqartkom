                ...
                <tbody class="text-center">
                    @foreach ($bids as $bid)
                        <tr class="intro-x">
                            ...
                            <td class="w-40">
                                <a href="" class="font-medium whitespace-nowrap">{{ $bid->name }}</a>
                            </td>
                            <td class="w-40">
                                <a href="" class="font-medium whitespace-nowrap">{{ $bid->phone }}</a>
                            </td>
                            <td class="w-40">
                                <a href="" class="font-medium whitespace-nowrap">{{ $bid->national_number }}</a>
                            </td>
                            <td class="w-40">
                                <a href="" class="font-medium whitespace-nowrap">{{ $bid->amount }}</a>
                            </td>
                            <td class="w-40">
                                <a href=""
                                    class="font-medium whitespace-nowrap">{{ __('admin.approved_statuses')[$bid->status] }}</a>
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-start items-center">
                                    @php $request=$bid @endphp
                                    @if ($bid->status == 'pending')
                                        @include('auction::partials.accept-cancel-buttons')
                                    @endif
                                    <a class="flex items-center mr-3"
                                        href="{{ route('dashboard.bid.edit', ['bid' => $bid->id, 'auction' => $auction->uuid]) }}">
...
@section('script')
    <script>
        ...
        $(".deleteBtn").on("click", function() {
            var id = $(this).data('id');
            $('#deleteForm').attr('action', `/dashboard/auction/{{ $auction->uuid }}/bid/${id}`);
        });
    </script>
