...
@section('content')
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{ route('dashboard.ad-feature-package.create') }}"
            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="text-center whitespace-nowrap">{{ __('validation.attributes.days') }}</th>
                        <th class="text-center whitespace-nowrap">{{ __('validation.attributes.type') }}</th>
                ...
                <tbody class="text-center">
                    @foreach ($packages as $package)
                        <tr class="intro-x">
                            ...
                            <td class="w-40">
                                <a href="" class="font-medium whitespace-nowrap">{{ $package->days }}</a>
                            </td>
                            <td class="w-40">
                                <a href=""
                                    class="font-medium whitespace-nowrap">{{ __('admin.users')[$package->type] }}</a>
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">

                                    <a class="flex items-center mr-3"
                                        href="{{ route('dashboard.ad-feature-package.edit', ['package' => $package->id]) }}">
...
@section('script')
    <script>
        ...
        $(".deleteBtn").on("click", function() {
            var id = $(this).data('id');
            $('#deleteForm').attr('action', `/dashboard/ad-feature-package/${id}`);
        });
    </script>
@endsection
