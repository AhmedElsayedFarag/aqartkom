                ...
                <tbody class="text-center">
                    @foreach ($cities as $city)
                        <tr class="intro-x">
                            ...
                            <td class="w-40">
                                <a href="" class="font-medium whitespace-nowrap">{{ $city->name }}</a>
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3"
                                        href="{{ route('dashboard.neighborhood.index', $city->id) }}">
                                        <i data-feather="layers" class="w-4 h-4 mr-1"></i>
                                        احياء
                                    </a>
                                    <a class="flex items-center mr-3" href="{{ route('dashboard.city.edit', $city->id) }}">
...
@section('script')
    <script>
        ...
        $(".deleteBtn").on("click", function() {
            var id = $(this).data('id');
            $('#deleteForm').attr('action', `/dashboard/city/${id}`);
        });
    </script>
@endsection
