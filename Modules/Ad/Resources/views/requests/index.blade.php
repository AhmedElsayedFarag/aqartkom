            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="text-center whitespace-nowrap">{{ __('validation.attributes.category') }}</th>
                        <th class="text-center whitespace-nowrap">{{ __('validation.attributes.city') }}</th>
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-start items-center">
                                    ...
                                    {{-- <a class="flex items-center mr-3"
                                        href="{{ route('dashboard.ad-request.edit', $ad->uuid) }}"
                                    ...
                                    @if ($ad->status->value == 'pending' || $ad->status->value == 'cancelled')
                                        <a class="acceptBtn flex items-center text-success mr-3" href="javascript:;"
                                            data-tw-toggle="modal" data-tw-target="#accept-confirmation-modal"
                                            data-id="{{ $ad->uuid }}" title="{{ __('admin.approve') }}">
                                            <i data-feather="thumbs-up" class="w-4 h-4 ml-1"></i>

                                        </a>
                                    @endif
                                    @if ($ad->status->value == 'approved')
                                        <a href="#" class="mr-4 ml-4" />
                                    @endif
                                    @if ($ad->status->value == 'pending' || $ad->status->value == 'approved')
                                        <a class="cancelBtn flex items-center text-danger mr-3" href="javascript:;"
                                            title="{{ __('admin.cancel') }}" data-tw-toggle="modal"
                                            data-tw-target="#cancel-confirmation-modal" data-id="{{ $ad->uuid }}">
...
@section('script')
    <script>
        function reset() {
            ...
            $('#category').val(' ');
            $('#type').val(' ');
            location.href = '{{ route('dashboard.ad-request.index') }}';
        }
        ...
        $(".deleteBtn").on("click", function() {
            var id = $(this).data('id');
            $('#deleteForm').attr('action', `/dashboard/ad-request/${id}`);
        });
        $(".terminateBtn").on("click", function() {
            var id = $(this).data('id');
            $('#terminateForm').attr('action', `/dashboard/ad-request/${id}/terminate`);
        });
        $(".cancelBtn").on("click", function() {
            var id = $(this).data('id');
            $('#cancelForm').attr('action', `/dashboard/ad-request/${id}/cancel`);
        });
        $(".acceptBtn").on("click", function() {
            var id = $(this).data('id');
            $('#acceptForm').attr('action', `/dashboard/ad-request/${id}/accept`);
        });
    </script>
@endsection
