...
@section('content')
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            ...
            <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0 mr-2">
                <a class=" flex items-center btn btn-primary" href="{{ route('dashboard.customer.export') }}">
        ...
        <div class="intro-y col-span-12 overflow-auto ">
            <table class="table table-report -mt-2">
                ...
                <tbody class="text-center">
                    @foreach ($customers as $index => $customer)
                        <tr class="intro-x">
                            <td class="w-40">
                                <a href="{{ route('dashboard.customer.show', $customer->uuid) }}"
                            ...
                            <td class="w-40">
                                <a href="{{ route('dashboard.customer.show', $customer->uuid) }}"
                                    class="font-medium whitespace-nowrap"
                                    style="word-break: break-all;">{{ $customer->name }}</a>
                            </td>
                            <td class="w-40">
                            ...
                            <td class="w-40">
                                <div class="flex items-center justify-center">
                                    {{ $customer->is_authorized == 1 ? __('admin.is_authorized') : __('admin.not_authorized') }}
                                </div>
                            </td>
                            <td class="w-40">
                                <div class="flex items-center justify-center">
                                    {{ __('admin.block_status')[$customer->is_blocked] }}
                                </div>
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="btn btn-warning "
                                        href="{{ route('dashboard.customer.show', $customer->uuid) }}"
                                        title="{{ __('admin.show', ['attribute' => __('admin.users.customer')]) }}">
                                        <i data-feather="eye" class="w-4 h-4 mr-1"></i>
                                    </a>
                                    {{-- <a class="flex items-center mr-3"
                                        href="{{ route('dashboard.customer.edit', $customer->uuid) }}"> <i
                                    ...
                                    <a class="sendMessageBtn flex items-center " href="javascript:;" data-tw-toggle="modal"
                                        data-tw-target="#send-user-message-modal" data-id="{{ $customer->uuid }}">
                                    ...
                                    <a class="blockBtn flex items-center @if (!$customer->is_blocked) text-danger @else text-success @endif"
                                        href="javascript:;" data-tw-toggle="modal"
                                        data-tw-target="#block-confirmation-modal" data-id="{{ $customer->uuid }}">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-body p-0">
                            ...
                            <div class="px-5 pb-8 text-center">
                                <form id="sendForm" action="{{ route('dashboard.customer.send-topic') }}"
                                    method="POST">
                                    @csrf
                                    <div class="input-form">
                                        ...
                                        @error('title')
                                            <div class="pristine-error text-danger mt-2">{{ $errors->first('title') }}
                                            </div>
                                        @enderror
                                    </div>
                                    <div class="input-form">
            ...
            <div id="send-user-message-modal" class="modal" tabindex="-1" aria-hidden="true"
                style="padding-left: 0px;">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-body p-0">
