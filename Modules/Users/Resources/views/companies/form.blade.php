                                <div class="input-form">
                                    <label for="validation-form-1" class="form-label w-full flex flex-col sm:flex-row">
                                        {{ __('validation.attributes.image') }}
                                        {{-- <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-slate-500">Required, at least 3 characters</span> --}}
                                    </label>
                                    <input id="validation-form-1" type="file" name="profile_image" class="form-control"
                                        accept="image/*">
                                    @error('profile_image')
                                        <div class="pristine-error text-danger mt-2">{{ $errors->first('profile_image') }}
                                        </div>
                                    @enderror
                                    <img src="{{ $user->formatted_profile }}" class="w-1/4 mb-4 mt-4" />
                                </div>
                                <div class="input-form">
                                    <label for="validation-form-1" class="form-label w-full flex flex-col sm:flex-row">
                                        {{ __('validation.attributes.name') }}
                                        {{-- <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-slate-500">Required, at least 3 characters</span> --}}
                                    </label>
                                    <input id="validation-form-1" type="text" name="name" class="form-control"
                                        placeholder="John Legend" minlength="3"
                                        value="{{ $user->name ?? old('name') }}" required>
                                    @error('name')
                                        <div class="pristine-error text-danger mt-2">{{ $errors->first('name') }}</div>
                                    @enderror
                                </div>
                                <div class="input-form mt-3">
                                    <label for="validation-form-1" class="form-label w-full flex flex-col sm:flex-row">
                                        {{ __('validation.attributes.email') }}
                                        {{-- <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-slate-500">Required, valid email</span> --}}
                                    </label>
                                    <input id="validation-form-1" type="email" name="email" class="form-control"
                                        placeholder="hello@example.com" minlength="3"
                                        value="{{ $user->email ?? old('email') }}" required>
                                    @error('email')
                                        <div class="pristine-error text-danger mt-2">{{ $errors->first('email') }}</div>
                                    @enderror
                                </div>
                                <div class="input-form mt-3">
                                    <label for="validation-form-1" class="form-label w-full flex flex-col sm:flex-row">
                                        {{ __('validation.attributes.phone') }}
                                        {{-- <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-slate-500">Required, valid phone number</span> --}}
                                    </label>
                                    <div class="input-group">
                                        <input id="validation-form-1" type="text" name="phone" class="form-control"
                                            placeholder="52345678" pattern="(5|0|3|6|4|9|1|8|7)([0-9]{7})"
                                            aria-describedby="input-group-2"
                                            value="{{ isset($user) ? \str_replace('+9665', '', $user->phone) : old('phone') }}"
                                            required>

                                        <div id="input-group-3" class="input-group-text">9665+</div>
                                        @error('phone')
                                            <div class="pristine-error text-danger mt-2">{{ $errors->first('phone') }}
                                            </div>
                                        @enderror
                                    </div>

                                </div>
                                <div class="input-form mt-3">
                                    <label for="validation-form-1" class="form-label w-full flex flex-col sm:flex-row">
                                        {{ __('validation.attributes.whatsapp') }}
                                        {{-- <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-slate-500">Required, valid whatsapp_number number</span> --}}
                                    </label>
                                    <div class="input-group">
                                        <input id="validation-form-1" type="text" name="whatsapp_number"
                                            class="form-control" placeholder="52345678"
                                            pattern="(5|0|3|6|4|9|1|8|7)([0-9]{7})" aria-describedby="input-group-2"
                                            value="{{ isset($user) ? \str_replace('+9665', '', $user->companyProfile->whatsapp_number) : old('whatsapp_number') }}"
                                            required>

                                        <div id="input-group-3" class="input-group-text">9665+</div>
                                        @error('whatsapp_number')
                                            <div class="pristine-error text-danger mt-2">
                                                {{ $errors->first('whatsapp_number') }}</div>
                                        @enderror
                                    </div>

                                </div>

                                <div class="input-form mt-3">
                                    <label for="validation-form-1" class="form-label w-full flex flex-col sm:flex-row">
                                        {{ __('validation.attributes.commercial_register_number') }}
                                        {{-- <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-slate-500">Required, valid phone number</span> --}}
                                    </label>
                                    <div class="input-group">
                                        <input id="validation-form-1" type="number" name="commercial_register_number"
                                            class="form-control" placeholder="52345678" aria-describedby="input-group-2"
                                            value="{{ isset($user) ? $user->companyProfile->commercial_register_number : old('commercial_register_number') }}"
                                            required>
                                        @error('commercial_register_number')
                                            <div class="pristine-error text-danger mt-2">
                                                {{ $errors->first('commercial_register_number') }}
                                            </div>
                                        @enderror
                                    </div>

                                </div>
                                <div class="mt-3">
                                    <label for="city-select"
                                        class="form-label">{{ __('validation.attributes.city') }}</label>
                                    <select data-placeholder="اختر" name="city_id" class="tom-select w-full"
                                        id="city-select">
                                        <option value=""></option>
                                        @foreach ($cities as $city)
                                            <option value="{{ $city->id }}"
                                                @isset($user){{ $user->companyProfile->city_id == $city->id ? 'selected' : '' }}@endisset>
                                                {{ $city->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('city_id')
                                        <div class="pristine-error text-danger mt-2">{{ $errors->first('city_id') }}</div>
                                    @enderror
                                </div>
                                <div class="grid grid-cols-2 gap-2 permissions mt-4">
                                    <div class="w-50">
                                        <div class="row">
                                            <img src="{{ asset($user->companyProfile->logo) }}" class="mb-3"
                                                style="width:200px;height:200px;" id="back_preview" />
                                        </div>
                                        <div class="form-group row">
                                            <label for="image"
                                                class="col-md-2 col-form-label">{{ __('validation.attributes.image') }}</label>
                                            <div class="col-md-10">
                                                <input class="form-control @error('image') is-invalid @enderror"
                                                    type="file" accept="image/*" id="back_input" name="image"
                                                    required>
                                                @error('image')
                                                    <span class="invalid-feedback" role="alert">
                                                        {{ $message }}
                                                    </span>
                                                @enderror
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                @include('partials.map-component')
                                @push('scriptsStack')
                                    <script>
                                        let backImg = document.getElementById('back_preview');
                                        let backInp = document.getElementById('back_input');
                                        backInp.onchange = evt => {
                                            console.log(evt);
                                            const [file] = backInp.files
                                            if (file) {
                                                backImg.src = URL.createObjectURL(file)
                                            }
                                        }
                                    </script>
                                    @include('partials.map-scripts')
                                @endpush
