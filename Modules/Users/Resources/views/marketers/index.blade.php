...
@section('content')
    <div class="grid grid-cols-12 gap-6 mt-5">
            ...
            <table class="table table-report -mt-2">
                ...
                <tbody class="text-center">
                    @foreach ($marketers as $index => $marketer)
                        <tr class="intro-x">
                            <td class="w-40">
                                <a href="{{ route('dashboard.marketer.show', $marketer->uuid) }}"
                            ...
                            <td class="w-40">
                                <a href="{{ route('dashboard.marketer.show', $marketer->uuid) }}"
                                    class="font-medium whitespace-nowrap"
                                    style="word-break: break-all;">{{ $marketer->name }}</a>
                            </td>
                            <td class="w-40">
                            ...
                            <td class="w-40">
                                <div class="flex items-center justify-center">
                                    {{ $marketer->is_authorized == 1 ? __('admin.is_authorized') : __('admin.not_authorized') }}
                                </div>
                            </td>
                            <td class="w-40">
                                <div class="flex items-center justify-center">
                                    {{ __('admin.block_status')[$marketer->is_blocked] }}
                                </div>
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="btn btn-warning "
                                        href="{{ route('dashboard.marketer.show', $marketer->uuid) }}?ad-status=not-licensed"
                                        title="{{ __('admin.show', ['attribute' => __('admin.users.marketer')]) }}">
                                        <i data-feather="eye" class="w-4 h-4 mr-1"></i>
                                    </a>
                                    {{-- <a class="flex items-center mr-3"
                                        href="{{ route('dashboard.owner.edit', $marketer->uuid) }}"> <i
                                    ...
                                    <a class="sendMessageBtn flex items-center " href="javascript:;" data-tw-toggle="modal"
                                        data-tw-target="#send-user-message-modal" data-id="{{ $marketer->uuid }}">
                                    ...
                                    <a class="deleteBtn flex items-center text-danger" href="javascript:;"
                                        ...
                                        data-id="{{ $marketer->uuid }}">
                                        <i data-feather="trash-2" class="w-4 h-4 mr-1"></i>
                                        {{ __('admin.delete', ['attribute' => '']) }}
                                    </a>
                                    <a class="blockBtn flex items-center @if (!$marketer->is_blocked) text-danger @else text-success @endif"
                                        href="javascript:;" data-tw-toggle="modal"
                                        data-tw-target="#block-confirmation-modal" data-id="{{ $marketer->uuid }}">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-body p-0">
                            ...
                            <div class="px-5 pb-8 text-center">
                                <form id="sendForm" action="{{ route('dashboard.marketer.send-topic') }}"
                                    method="POST">
                                    @csrf
        ...
        {{ $marketers->links('vendor.pagination.tailwind') }}
    </div>
@endsection
