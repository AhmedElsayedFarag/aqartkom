            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="text-center whitespace-nowrap">{{ __('validation.attributes.ad') }}</th>
                ...
                <tbody class="text-center">
                    @foreach ($banners as $banner)
                        <tr class="intro-x">
                            <td class="w-40">
                                <img src="{{ $banner->formattedUrl }}" />
                            </td>
                            <td class="w-40">
                                <a href="" class="font-medium whitespace-nowrap">{{ $banner->ad->estate->title }}</a>
                            </td>
...
@section('script')
    <script>
        ...
        $(".deleteBtn").on("click", function() {
            var id = $(this).data('id');
            $('#deleteForm').attr('action', `/dashboard/banner/${id}`);
        });
    </script>
@endsection
