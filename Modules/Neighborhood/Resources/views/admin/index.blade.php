...
@section('content')
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{ route('dashboard.neighborhood.create', ['city' => $city->id]) }}"
                ...
                <tbody class="text-center">
                    @foreach ($neighborhoods as $neighborhood)
                        <tr class="intro-x">
                            ...
                            <td class="w-40">
                                <a href="" class="font-medium whitespace-nowrap">{{ $neighborhood->name }}</a>
                            </td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">

                                    <a class="flex items-center mr-3"
                                        href="{{ route('dashboard.neighborhood.edit', ['city' => $city->id, 'neighborhood' => $neighborhood->id]) }}">
        ...
        {{ $neighborhoods->links('vendor.pagination.tailwind') }}
    </div>
@endsection
...
@section('script')
    <script>
        ...
        $(".deleteBtn").on("click", function() {
            var id = $(this).data('id');
            $('#deleteForm').attr('action', `/dashboard/city/{{ $city->id }}/neighborhood/${id}`);
        });
    </script>
@endsection
